  ...
  defmodule Oban.Web.ProFixtures do
    ...
    defmodule ChunkWorker do
      ...
      def process_chunk(jobs) do
        if Enum.any?(jobs, & &1.args["block"]), do: Oban.Web.ProFixtures.block_chunk()
    ...
    @chunk_listener :oban_web_chunk_listener

    def run_chunk!(args_list) do
    ...
    def start_blocked_chunk!(args_list) do
      Process.register(self(), @chunk_listener)

      changesets = Enum.map(args_list, &ChunkWorker.new(Map.put(&1, :block, true)))
      last_id = last_job_id()
    ...
    def start_waiting_chunk!(args) do
    ...
    def block_chunk do
      send(@chunk_listener, {:chunk_running, self()})
    ...
    def decorated_cron_entry(expression, opts \\ []) do
      entry_opts =
        [
          args: %{mod: inspect(Reports), fun: "digest", arg: []},
          meta: %{decorated: true, decorated_name: decorated_cron_name()}
        ]
        |> Keyword.merge(opts)
    ...
    def decorated_cron_name, do: "#{inspect(Reports)}.digest/0"
