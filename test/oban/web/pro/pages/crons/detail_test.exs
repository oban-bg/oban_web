  ...
  defmodule Oban.Web.Pro.Pages.Crons.DetailTest do
    ...
    describe "decorated crons" do
      setup do
        ...
        {:ok, handler: handler, live: live}
      end

      test "presenting the decorated function as the handler", %{handler: handler, live: live} do
        html = refresh(live)
      ...
      test "run now preserves the decorator args and labels the job", %{
        handler: handler,
        live: live
      } do
        refresh(live)
        ...
        assert %{"mod" => mod, "fun" => "digest", "arg" => _} = job.args
        assert handler == "#{mod}.digest/0"
      ...
      test "editing the schedule leaves the handler untouched", %{handler: handler, live: live} do
        ...
        assert %{"mod" => _, "fun" => "digest", "arg" => _} = entry.opts["args"]
      end
    end
