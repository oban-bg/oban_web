defmodule Oban.Web.Queues.DetailComponentTest do
  ...
  test "restricting actions based on access" do
    ...
    assert has_fragment?(html, "input[name=local_limit][disabled]")
    assert has_fragment?(html, "input[name=global_allowed][disabled]")
    assert has_fragment?(html, "input[name=rate_allowed][disabled]")

    html = render_component(Component, assigns(access: :all), router: Router)

    refute has_fragment?(html, "input[name=local_limit][disabled]")
  end
