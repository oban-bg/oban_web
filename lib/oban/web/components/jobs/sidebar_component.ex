  ...
  def render(assigns) do
    ...
    <div id="sidebar" class="mr-0 mb-3 md:mr-3 md:mb-0">
      <div id="nodes" class="bg-white dark:bg-gray-900 w-fill mb-3 rounded-md shadow-lg overflow-hidden md:w-84">
        ...
        <ul class={if @show_nodes?, do: "block", else: "hidden"}>
          <%= for {name, stat} <- @node_stats do %>
            <.live_component id={name} module={NodeComponent} params={@params} name={name} stat={stat} />
          <% end %>
        </ul>
      </div>
      ...
      <div id="states" class="bg-white dark:bg-gray-900 w-full mb-3 rounded-md shadow-lg overflow-hidden md:w-84">
        ...
        <ul class={if @show_states?, do: "block", else: "hidden"}>
          <%= for {name, stat} <- @state_stats do %>
            <.live_component id={name} module={StateComponent} params={@params} name={name} stat={stat} />
          <% end %>
        </ul>
      </div>
      ...
      <div id="queues" class="bg-white dark:bg-gray-900 w-full rounded-md shadow-lg overflow-hidden md:w-84">
        ...
        <ul class={if @show_queues?, do: "block", else: "hidden"}>
          <%= for {name, stat} <- @queue_stats do %>
            <.live_component id={name} module={QueueComponent} access={@access} params={@params} name={name} stat={stat} />
          <% end %>
        </ul>
      </div>
    </div>
