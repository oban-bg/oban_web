...
**The Series Palette Rule.** When a chart groups by something other than state (queue, node, worker), series take hues that never appear in the state palette — orange, teal, fuchsia, lime, sky, pink, amber, red (`Oban.Web.Colors.series_colors/1`) — and draw one register away from it: 600 in light mode, 300 in dark mode, never 400. Neighbouring hues (sky beside cyan, teal beside emerald) still read as a different family because the weight differs, so a queue can never be misread as a state. Bars, lines, tooltip dots, and legend dots share the register. Each label hashes to a stable hue, so a series keeps its color across refreshes and groupings; series are stacked and listed alphabetically so their positions never move. Anything past the top seven folds into a gray `other` series, and a series already on the chart keeps its slot until a challenger clearly outranks it.
...
Two grammars for two filter semantics. **Exclusive** rows (job states) are links: the active row lifts onto a white / gray-800 fill with `rounded-md` and bolder text, and hover previews the same fill in gray-100. **Additive** rows (nodes, queues) are toggle buttons (`aria-pressed`) with a 4px transparent left border: active rows snap the border to violet-500 and bolden; hover previews violet-400. Every row carries a gutter glyph: a state dot that dims to gray at zero, a node icon, or a queue-details link. Counts sit right-aligned in tabular gray text, stepping down to the Faint tier at zero, and each carries a visually hidden column label so rows read as "limit 30, executing 12" to assistive tech.

Column headers are quiet caps with a tooltip and hidden full label where they abbreviate (`EXEC`, `MODE`). The third queue column follows the selected state: its header is that state's dot and its values are that state's per-queue counts, so choosing `discarded` turns the queue list into an incident lens. When `executing` is selected the column shows `available` instead. The `MODE` column appears only when some queue carries a global limit, rate limit, partition, or pause; paused glyphs take the warning amber, configuration glyphs stay gray.

Sections collapse via a rotating chevron labelled "Collapse"/"Expand" with a `fade-in-scale` transition; the collapsed set persists to localStorage (`oban:sidebar_collapsed`) and is applied before first paint. Empty sections say why ("No nodes reporting"). The sidebar is an `aside` landmark, and its right edge carries a grab handle — a gray pill that glows violet on hover — for drag-resizing, exposed as a keyboard separator with a pixel value.
...
Inline SVG sparklines: cyan (#22d3ee) 4px bars with 1px gaps and 1px radius over gray-200/gray-700 placeholder stubs, tooltip on hover. Full charts (Chart.js) fill bars in the state palette's 400-level hexes in both themes and stroke lines at 400 in dark mode, stepping to the text levels in light mode (see The 400 Rule) — this is where the luminous half of the identity earns its keep. Axis ticks use the Muted tier, gridlines the Border tier, in both themes. Time labels are a 24-hour clock with seconds, placed on clock multiples of an interval chosen to leave each label room. The y-axis speaks in whole counts with `k`, `M`, and `B` suffixes, and in durations stepped through clock-shaped intervals (`ms`, `s`, `min`, `h`), so no tick ever lands on 0.4 jobs or 1.7 minutes. The chart is the sidebar extended in time: it owns only the metric, chosen with a three-segment radio control (`exec`, `time`, `wait`) whose checked segment lifts onto a white / gray-700 fill with semibold text, the sidebar's active-row grammar with no hue spent on it, and an options menu for grouping, period, and percentile. Full counts are not charted: they rarely change, completed jobs dominate them, and the sidebar already carries and filters them. Grouping is automatic by default: the state stack, unless a queue, node, or worker filter carries two or more values, which regroups by that dimension. Choosing a grouping outright compares every queue, node, or worker without picking them first. A selected state is isolated in the state stack: it keeps its hue at the baseline and every other series draws in the hidden-series gray (gray-300 light, gray-600 dark) so total volume still gives scale; with any other grouping the state narrows the query instead and the subtitle names it. Lines follow the same rule, thinned to 1px and ghosted around the emphasised series, with no point markers. State groupings carry no legend because the sidebar's state dots already map every hue. Queue, node, and worker groupings get a DOM legend under the plot in the sidebar's dot grammar: a 2px dot in the series hue, the label in gray text, and `aria-pressed` toggles that dim a hidden series to the gray zero-count dot; hovering or focusing a legend item ghosts the other series while it lasts. Hovering shows a tooltip beside the column rather than over it, so the segment a click would drill into stays visible; rows use the same dot grammar in the order the column stacks them, top first (lines list the highest value first), empty slices are omitted, and a muted footer names the segment under the pointer ("Click to filter by retryable") so the drill-through is discoverable before it happens. Clicking a bar or point drills into the jobs list filtered by that series. The plot is the room's one authored motion: bars sit at absolute times on a linear axis and the window glides left at one column per period, its right edge trailing server time by a period plus a refresh so every slice has ended and reported before it enters, arriving whole and sliding in rather than popping or growing. The y-axis holds its ceiling while bars scroll off and only drops once data falls below half of it, so the window is the only thing that moves. It stops when refresh is paused, when the tab is hidden, or under reduced motion, where the window steps only as data arrives. Emphasis changes (isolation, legend hover) crossfade over 200ms; values never animate.
